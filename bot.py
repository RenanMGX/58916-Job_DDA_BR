...
class Execute:
    @staticmethod
    def start():
        crd_label_param = execution.parameters.get('crd_label')
        if crd_label_param:
            crd_label = str(crd_label_param)
        else:
            raise ValueError(f"Parâmetro '{crd_label_param=}' não encontrado nos parâmetros da execução.")
        
        ExecuteAPP.start(
            user=maestro.get_credential(label=crd_label, key="user"),
            password=maestro.get_credential(label=crd_label, key="password"),
            ambiente=maestro.get_credential(label=crd_label, key="ambiente"),
            maestro=maestro
        )
